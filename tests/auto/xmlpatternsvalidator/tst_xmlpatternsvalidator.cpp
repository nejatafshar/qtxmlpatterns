...
tst_XmlPatternsValidator::tst_XmlPatternsValidator()
    : m_command(QLibraryInfo::location(QLibraryInfo::BinariesPath) + QLatin1StringView("/xmlpatternsvalidator"))
    , m_dontRun(false)
{
}
...
void tst_XmlPatternsValidator::xsdSupport_data() const
{
    ...
    QVERIFY(QFile::exists(path + QLatin1StringView("instance.xml")));
    ...
    QTest::newRow("A valid schema")
        << 0
        << QStringList(path + QLatin1StringView("valid_schema.xsd"))
        << QString();

    QTest::newRow("An invalid schema")
        << 1
        << QStringList(path + QLatin1StringView("invalid_schema.xsd"))
        << QString();

    QTest::newRow("An instance and valid schema")
        << 0
        << (QStringList() << path + QLatin1StringView("instance.xml")
                          << path + QLatin1StringView("valid_schema.xsd"))
        << QString();

    QTest::newRow("An instance and invalid schema")
        << 1
        << (QStringList() << path + QLatin1StringView("instance.xml")
                          << path + QLatin1StringView("invalid_schema.xsd"))
        << QString();

    QTest::newRow("An instance and not matching schema")
        << 1
        << (QStringList() << path + QLatin1StringView("instance.xml")
                          << path + QLatin1StringView("other_valid_schema.xsd"))
        << QString();

    QTest::newRow("Two instance documents")
        << 1
        << (QStringList() << path + QLatin1StringView("instance.xml")
                          << path + QLatin1StringView("instance.xml"))
        << QString();
    ...
    QTest::newRow("A schema aware valid instance document")
        << 0
        << (QStringList() << path + QLatin1StringView("sa_valid_instance.xml"))
        << QString();

    QTest::newRow("A schema aware invalid instance document")
        << 1
        << (QStringList() << path + QLatin1StringView("sa_invalid_instance.xml"))
        << QString();
    ...
    QTest::newRow("QTBUG-8394 A schema with an indirectly included type")
        << 0
        << (QStringList() << path + QLatin1StringView("indirect-include-a.xsd"))
        << QString();

    QTest::newRow("QTBUG-8394 A schema with an indirectly imported type")
        << 0
        << (QStringList() << path + QLatin1StringView("indirect-import-a.xsd"))
        << QString();

    QTest::newRow("QTBUG-8394 A schema with an indirectly redefined type")
        << 0
        << (QStringList() << path + QLatin1StringView("indirect-redefine-a.xsd"))
        << QString();

    QTest::newRow("QTBUG-8920 A schema with a complex type that indirectly includes an anonymous type")
        << 0
        << (QStringList() << path + QLatin1StringView("complex-type-including-anonymous-type.xsd"))
        << QString();

    QTest::newRow("QTBUG-11559 A schema and instance with a dateTime containing microseconds")
        << 0
        << (QStringList() << path + QLatin1StringView("dateTime-with-microseconds.xml")
                          << path + QLatin1StringView("dateTime-with-microseconds.xsd"))
        << QString();

    QTest::newRow("A document with a valid substitution group")
        << 0
        << (QStringList() << path + QLatin1StringView("substitution-group-valid.xml")
                          << path + QLatin1StringView("substitution-group.xsd"))
        << QString();

    QTest::newRow("A document attempting to use a prohibited substitution")
        << 1
        << (QStringList() << path + QLatin1StringView("substitution-group-invalid.xml")
                          << path + QLatin1StringView("substitution-group.xsd"))
        << QString();
    QTest::newRow("QTBUG-58245 fraction digits should ignore trailing 0")
            << 0
            << (QStringList() << path + QLatin1StringView("fractiondigits.xml")
                              << path + QLatin1StringView("fractiondigits.xsd"))
            << QString();
    QTest::newRow("QTBUG-58245 fraction digits should ignore trailing 0 with no dots")
            << 0
            << (QStringList() << path + QLatin1StringView("fractiondigits-nodot.xml")
                              << path + QLatin1StringView("fractiondigits.xsd"))
            << QString();
    QTest::newRow("QTBUG-58245 fraction digits should ignore trailing 0 with no number before dot")
            << 0
            << (QStringList() << path + QLatin1StringView("fractiondigits-nonumber.xml")
                              << path + QLatin1StringView("fractiondigits.xsd"))
            << QString();
    QTest::newRow("QTBUG-58245 fraction digits should ignore trailing 0 invalid")
            << 1
            << (QStringList() << path + QLatin1StringView("fractiondigits-invalid.xml")
                              << path + QLatin1StringView("fractiondigits.xsd"))
            << QString();
}
